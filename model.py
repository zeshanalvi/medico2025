...
class VQAModel(nn.Module):
    ...
    def fine_tune(self,
              ...
              validate_every=1):
        """
        Fine-tune the model without changing other functions.
        - train_loader: DataLoader for training
        - val_loader: DataLoader for validation (optional)
        - epochs: number of fine-tuning epochs
        - lr: learning rate for optimizer
        - unfreeze_encoders: if True, include encoders (image & question & disease) in training
        - save_best_path: path to save best checkpoint (optional)
        - validate_every: run validation every N epochs
        """

        # Preconditions
        assert hasattr(self, "fusion_module"), "fusion_module is missing"
        assert hasattr(self, "qtype_classifier"), "qtype_classifier is missing"
        assert hasattr(self, "task_heads"), "task_heads (ModuleDict) is missing"
        assert hasattr(self, "answer_vocabs"), "answer_vocabs must exist (build before fine-tuning)"

        device = self.device

        # 1. Freeze or unfreeze encoders
        def set_requires_grad(module, value):
            for p in module.parameters():
                p.requires_grad = value

        if not unfreeze_encoders:
            # Freeze heavy encoders by default so we fine-tune heads + fusion
            if hasattr(self, "image_encoder"):
                set_requires_grad(self.image_encoder, False)
            if hasattr(self, "question_encoder"):
                set_requires_grad(self.question_encoder, False)
            if hasattr(self, "disease_model"):
                set_requires_grad(self.disease_model, False)
        else:
            # Ensure encoders are trainable
            if hasattr(self, "image_encoder"):
                set_requires_grad(self.image_encoder, True)
            if hasattr(self, "question_encoder"):
                set_requires_grad(self.question_encoder, True)
            if hasattr(self, "disease_model"):
                set_requires_grad(self.disease_model, True)

        # 2. Build optimizer parameter list
        params = list(self.fusion_module.parameters()) + list(self.qtype_classifier.parameters())
        # add all task heads
        for head in self.task_heads.values():
            params += list(head.parameters())

        # optionally include encoders if unfreezing
        if unfreeze_encoders:
            if hasattr(self, "image_encoder"):
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                loss = compute_loss(
                    ...
                    answers=answers,
                    task_logits=task_logits,
                    true_q_classes=batch["question_classes"],
                    answer_vocabs=self.answer_vocabs,
                    q_types_mapping=self.q_types_mapping,
                    q_types=self.q_types,
                    task_heads=self.task_heads
            ...
            print(f"[Fine-tune] Epoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.6f}")
            ...
            if val_loader is not None and ((epoch + 1) % validate_every == 0):
                ...
                print(f"[Fine-tune] Epoch {epoch+1}/{epochs} - Val Loss: {avg_val_loss:.6f}")

                # Save best
                if save_best_path is not None and avg_val_loss < best_val_loss:
                    best_val_loss = avg_val_loss
                    best_epoch = epoch + 1

                    # Build checkpoint that matches your load() expectations
                    checkpoint = {
                        "fusion_module": self.fusion_module.state_dict(),
                        "question_encoder": self.question_encoder.state_dict(),
                        "image_encoder": self.image_encoder.state_dict(),
                        ...
                        "epochs": epoch + 1,
                        "answer_vocabs": self.answer_vocabs,
                        "task_vocabs": getattr(self, "task_vocabs", None)
                    }
                    torch.save(checkpoint, save_best_path)
                    print(f"[Fine-tune] Saved best checkpoint (epoch {best_epoch}) to {save_best_path}")

        # 5. restore encoders to trainable state if you changed them earlier (optional)
        if not unfreeze_encoders:
            # by default leave them frozen (as we started). If you prefer to unfreeze again after finetune,
            # set requires_grad True here.
            pass
